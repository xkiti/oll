...
```
# 四、世面上其他App架构的优缺点
架构服务于业务没有绝对的好坏。我这边先把把市面上的App架构归结为两种然后针对进行简单分析。
### 1. 超级App架构
超级App有两种，一种是集合型：聚集多个单一App功能的项目(安居客，平安银行等)，另一种是虚拟机型：本身含有类似虚拟机功能的App(微信小程序、支付宝小程序等)。
  
|        | 集合型    |虚拟机型|
| ----- | ----- |-----|
|   优点    | 拥有跨项目的扩展性，较高的代码利用率  |  比肩操作系统的项目扩展性，扩展项目具有一定的可控性 |
|    缺点    | 安装包大、编译速度慢、插件化受到系统版本限制、存在冗余功能等、统一性能监控难实现 、项目协作时间长 |技术门槛高、开发周期长、投入成本大、架构成熟周期长、容器内应用性能受限|
### 2. 单一App架构
世面上的App大部分是单一架构这边也主要分为：有动态化能力和无动态化能力。
相比较超级App具有的共同优点有：项目可以快速启动、安装包体积能稳步增长。
|       |   有动态化能力  |无动态化能力
| ----- | ----- |-----|
|    优点    |  可以有效减少发布小功能版本的次数，具有一定的修复能力  |  聚焦于业务功能实现，最快速度完成初版上线  |
|    缺点    |  有些方案存在系统兼容性问题，有些方案存在系统性能问题  |  扩展性偏低，没有修复能力，每次都需要发版本  |
# 结尾
